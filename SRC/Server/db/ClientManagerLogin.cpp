...
void CClientManager::QUERY_LOGIN_BY_KEY(CPeer * pkPeer, DWORD dwHandle, TPacketGDLoginByKey * p)
{
	...
	char szQuery_lang[QUERY_MAX_LEN];
	snprintf(szQuery_lang, sizeof(szQuery_lang), "SELECT lang FROM account.account WHERE id=%u", pkTab->id);
	std::unique_ptr<SQLMsg> pMsg(CDBManager::instance().DirectQuery(szQuery_lang));
...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer * peer, SQLMsg * msg)
{
	...
	if (g_stLocale == "gb2312")
	{
		...
		snprintf(szQuery, sizeof(szQuery),
				"SELECT id, name, job, level, alignment, prestige, lkill_point, st, ht, dx, iq, part_main, part_hair, part_acce, part_aura, x, y, skill_group, change_name FROM player%s WHERE account_id=%u",
	...
	{
		snprintf(szQuery, sizeof(szQuery),
				"SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_acce, part_aura, x, y, skill_group, change_name FROM player%s WHERE account_id=%u",
				GetTablePostfix(), info->pAccountTable->id);
	}
...
#ifdef ENABLE_MULTILANGUAGE
	str_to_number(pkTab->blang, row[col++]);
#endif
	...
	for (DWORD i = 0; i < mysql_num_rows(pRes); ++i)
	{
		...
		for (j = 0; j < PLAYER_PER_ACCOUNT; ++j)
		{
			if (pkTab->players[j].dwID == player_id)
			{
				...
				sys_log(0, "%s %lu %lu hair %u acce %u",
						pkTab->players[j].szName, pkTab->players[j].x, pkTab->players[j].y, pkTab->players[j].wHairPart, pkTab->players[j].wAccePart);
				break;
			}
		}
		/*
		...
		else
		{
			...
			if (g_stLocale == "gb2312")
			{
				snprintf(queryStr, sizeof(queryStr),
						"SELECT id, name, job, level, alignment, st, ht, dx, iq, part_main, part_hair, part_acce, part_aura, x, y, skill_group, change_name FROM player%s WHERE account_id=%u",
						GetTablePostfix(), info->pAccountTable->id);
			}
			else
			{
