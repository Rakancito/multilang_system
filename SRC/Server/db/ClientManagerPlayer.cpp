...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	else
	{
		...
		snprintf(queryStr, sizeof(queryStr),
				...
				"skill_level,quickslot,skill_group,alignment,prestige,lkill_point,mobile,horse_level,horse_riding,horse_hp,horse_hp_droptime,horse_stamina,"
...
void CClientManager::QUERY_ACCOUNT_CHANGE_LANGUAGE(CPeer * peer, DWORD dwHandle, TPacketGDChangeLanguage * data)
{
	CLoginData *pkLD = GetLoginDataByAID(data->dwAccountID);

	if (pkLD)
	{
		TAccountTable rAccountTable = pkLD->GetAccountRef();
		pkLD->GetAccountRef().blang = data->blang;
	}
}
#endif
...
bool CreatePlayerTableFromRes(MYSQL_RES * res, TPlayerTable * pkTab)
{
	...
	str_to_number(pkTab->lkill_point, row[col++]);			
#endif				
...
void CClientManager::__QUERY_PLAYER_CREATE(CPeer *peer, DWORD dwHandle, TPlayerCreatePacket* packet)
{
	...
	queryLen = snprintf(queryStr, sizeof(queryStr),
			...
			"job, voice, dir, x, y, z, "
			"hp, mp, random_hp, random_sp, stat_point, stamina, part_base, part_main, part_hair, part_acce,part_aura, gold, playtime, "
