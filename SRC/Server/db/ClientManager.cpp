...
void CClientManager::QUERY_AUTH_LOGIN(CPeer * pkPeer, DWORD dwHandle, TPacketGDAuthLogin * p)
{
	...
	else
	{
		...
		strlcpy(r.passwd, "TEMP", sizeof(r.passwd));
#ifdef ENABLE_MULTILANGUAGE
		sys_log(0, "AUTH_LOGIN id(%u) login(%s) social_id(%s) login_key(%u), client_key(%u %u %u %u), lang(%d)",
				p->dwID, p->szLogin, p->szSocialID, p->dwLoginKey,
				p->adwClientKey[0], p->adwClientKey[1], p->adwClientKey[2], p->adwClientKey[3], r.blang);
#else
	...
	while (peer->PeekPacket(i, header, dwHandle, dwLength, &data))
	{
		...
		switch (header)
		{
			...
			case HEADER_GD_HAMMER_OF_TOR:
				break;
#ifdef ENABLE_MULTILANGUAGE
			case HEADER_GD_CHANGE_LANGUAGE:
				QUERY_ACCOUNT_CHANGE_LANGUAGE(peer, dwHandle, (TPacketGDChangeLanguage*)data);
				break;
#endif
			...
			case HEADER_GD_GUILD_DUNGEON_CD:
				...
				break;
			case HEADER_GD_GUILD_DUNGEON_ST: //Fix meley lair
...
void CClientManager::GuildDungeonST(TPacketGDGuildDungeonST* sPacket) //Fix meley lair
{
