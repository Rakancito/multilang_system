...
namespace quest
{
	...
	int _get_empire_privilege_string(lua_State* L)
	{
		...
		for (int type = PRIV_NONE + 1; type < MAX_PRIV_NUM; ++type)
		{
			...
			if (pkPrivEmpireData && pkPrivEmpireData->m_value)
			{
				...
				os << LC_TEXT(DEFAULT_LANGUAGE, c_apszPrivNames[type]) << " : " <<
	...
	int _get_guild_privilege_string(lua_State* L)
	{
		...
		for (int type = PRIV_NONE+1; type < MAX_PRIV_NUM; ++type)
		{
			...
			if (pPrivGuildData && pPrivGuildData->value)
			{
				...
				os << LC_TEXT(DEFAULT_LANGUAGE, c_apszPrivNames[type]) << " : " << pPrivGuildData->value << "%"
	...
	int _big_notice_in_map( lua_State* L )
	{
	...
	int _warp_all_to_village( lua_State * L )
	{
		...
		SendNoticeMap(LC_TEXT(DEFAULT_LANGUAGE, "잠시후 모두 마을로 이동됩니다."), iMapIndex, false);
	...
	struct FPurgeArea
	{
		...
		FPurgeArea(int a, int b, int c, int d, bool allowNPC, LPCHARACTER p)
			: x1(a), y1(b), x2(c), y2(d), allowNPC(allowNPC),
			ExceptChar(p)
		{}
...
#ifdef NEW_PET_SYSTEM
				if ((allowNPC && pChar->IsNPC()) || (!pChar->IsPet() && !pChar->IsNewPet() && (true == pChar->IsMonster() || true == pChar->IsStone())))
#else
				if ((allowNPC && pChar->IsNPC() || !pChar->IsPet()) && (pChar->IsMonster() || pChar->IsStone()))
#endif
				{
	...
	int _purge_area( lua_State* L )
	{
		...
		bool allowNPC = lua_toboolean(L, 5);
		...
		if (NULL != pSectree)
		{
			FPurgeArea func(x1, y1, x2, y2, allowNPC, CQuestManager::instance().GetCurrentNPCCharacterPtr());
...
#define NAN (INFINITY-INFINITY)
	...
	int _mysql_password(lua_State* L)
	{
