...
namespace quest
{
	...
	bool NPC::OnChat(PC& pc)
	{
...

		{
			...
			CQuestManager & mgrLang = CQuestManager::instance();
			BYTE bPClang;
			bPClang = mgrLang.GetCurrentCharacterPtr() ? mgrLang.GetCurrentCharacterPtr()->GetLang() : DEFAULT_LANGUAGE;

			os << ", '"<<LC_TEXT(bPClang, "닫기")<<"'";
			os << ")";
#else
			os << ", '"<<LC_TEXT("닫기")<<"'";
#endif
