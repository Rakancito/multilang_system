...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
...

	{
		...
		p.bLang = d->GetAccountTable().blang; 
#endif
		...
		while (it != i.end()) {
			...
			if (pc)
			{
				...
				if ((shop = pc->GetMyShop())) {
					if ((long long)pc->GetShopTime() < get_global_time() || shop->GetItemCount() <= 0)
					{
...
EVENTFUNC(fix_shop_event)
{
	...
	SQLMsg * pkMsg(DBManager::instance().DirectQuery("SELECT ps.id,ps.shop_id,ps.player_id,REPLACE('%s','#PLAYER_NAME#',p.name),ps.vnum,ps.count,ps.price,%s,%s,ps.transmutation FROM `player_shop_items` ps LEFT JOIN player p ON p.id=ps.player_id where not EXISTS(select name from player_shop WHERE id=ps.shop_id) and not ISNULL(p.name)", LC_TEXT(DEFAULT_LANGUAGE, "SHOP_NAME"), szSockets, szAttrs));
#else
	...
	if (pRes->uiNumRows>0)
	{
		...
		{
			DWORD id = 0, pid = 0, vnum = 0, count = 0;
			DWORD socket[ITEM_SOCKET_MAX_NUM],attr[ITEM_ATTRIBUTE_MAX_NUM][2];
			int col = 0;
...
#ifdef ENABLE_TRANSMUTATION
			sprintf(query, "%s, transmutation=%u", query, transmutation);
#endif
...
#endif
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "다른거래창이 열린상태에서는 창고를 열수가 없습니다." ) );
...
void CInputDB::SafeboxChangePasswordAnswer(LPDESC d, const char* c_pData)
{
	...
	if (p->flag)
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "<창고> 창고 비밀번호가 변경되었습니다."));
	}
	else
	{
		d->GetCharacter()->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "<창고> 기존 비밀번호가 틀렸습니다."));
	}
}
...
void CInputDB::MapLocations(const char * c_pData)
{
	...
	while (bCount--)
	{
		for (int i = 0; i < MAP_ALLOW_LIMIT; ++i)
		{
			...
			DBManager::instance().DirectQuery("DELETE FROM %smap_list where channel = %d and map_index=%d ", get_table_postfix(), g_bChannel, pLoc->alMaps[i]);
			DBManager::instance().DirectQuery("REPLACE INTO %smap_list VALUES(%d, %d, %d, %d)", get_table_postfix(), g_bChannel, pLoc->alMaps[i], pLoc->wPort, inet_addr(pLoc->szHost));	
#endif
...
void CInputDB::BillingExpire(const char * c_pData)
{
	...
	else
	{
		if ((p->dwRemainSeconds - d->GetBillingExpireSecond()) > 60)
		{
			...
			if (ch)
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "결재기간이 %d분 후 만료 됩니다."), (p->dwRemainSeconds / 60));
		}
	}
}
...
void CInputDB::Notice(const char * c_pData)
{
	...
	//SendNotice(LC_TEXT(DEFAULT_LANGUAGE, szBuf));
	SendNotice(szBuf);
}
...
int CInputDB::Analyze(LPDESC d, BYTE bHeader, const char * c_pData)
{
	...
	case HEADER_DG_GUILD_DUNGEON_CD:
		...
		break;
	case HEADER_DG_GUILD_DUNGEON_ST: //Fix Meley lair
...
void CInputDB::AddMonarchMoney(LPDESC d, const char * data )
{
	...
	{
		if (number(1, 100) > 95)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "현재 %s 국고에는 %u 의 돈이 있습니다"), EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire));
	}
}
...
void CInputDB::DecMonarchMoney(LPDESC d, const char * data)
{
	...
	if (ch)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "현재 %s 국고에는 %d 의 돈이 있습니다"), EMPIRE_NAME(Empire), CMonarch::instance().GetMoney(Empire));
	}
}
...
void CInputDB::TakeMonarchMoney(LPDESC d, const char * data)
{
	...
	if (!CMonarch::instance().DecMoney(Money, Empire))
	{
		...
		LPCHARACTER ch = d->GetCharacter();
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "국고에 돈이 부족하거나 돈을 가져올수 없는 상황입니다"));
	}
}
...
{
	char notice[256];
	snprintf(notice, sizeof(notice), LC_TEXT(DEFAULT_LANGUAGE, "%s의 군주가 %s 님으로 교체되었습니다."), EMPIRE_NAME(info->bEmpire), info->szName);
	SendNotice(notice);
}
		...
		while (it != i.end()) {
			...
			if (pc)
				if (pc->GetPrivShop() == p->shop_id) {
					CShopManager::instance().RemoveAllItemsFromMysql(pc->GetPlayerID());
					pc->DeleteMyShop();
					return;
				}
...
void CInputDB::GuildDungeonST(const char* c_pData) //Fix Meley lair
{
