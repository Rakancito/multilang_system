...
class CInputMain : public CInputProcessor
{
	...
	protected:
		...
		void		TransmutationManager(LPCHARACTER ch, const char* c_pData);
#endif
		...
		void		ItemDestroy(LPCHARACTER ch, const char * data); //Destruir item
...
#ifdef GROUP_MATCH
		void		GroupMatch(LPCHARACTER ch, const char * data);
#endif

#ifdef ENABLE_GEM_SYSTEM	
		int 		GemShop(LPCHARACTER ch, const char * data, size_t uiBytes);
#endif
		void		Shop2(LPCHARACTER ch, const char * data);
		...
		void		SetLanguage(LPCHARACTER ch, const void * c_pData);
#endif
...
protected:
	...
	*/
	...
	void		GuildDungeonST(const char* c_pData); //Fix Meley lair
	#endif
