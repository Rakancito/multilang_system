...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	if (Login_IsInChannelService(szLogin))
	{
		...
		DBManager::instance().DirectQuery("UPDATE account.account SET lang='%d' WHERE login='%s'", bLang, szLogin);

		DBManager::instance().ReturnQuery(QID_AUTH_LOGIN, dwKey, p,
				"SELECT '%s',password,securitycode,social_id,lang,id,status,availDt - NOW() > 0,"
