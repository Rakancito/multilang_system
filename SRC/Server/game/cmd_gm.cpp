...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if ((pkChr->GetRaceNum() != 30000 || pkChr->GetRaceNum() != 30001 ||
		pkChr->GetRaceNum() != 30002 || pkChr->GetRaceNum() != 30003 || pkChr->GetRaceNum() != 30004 ||
		pkChr->GetRaceNum() != 30005 ||	pkChr->GetRaceNum() != 30006 || pkChr->GetRaceNum() != 30007) && 
		pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewPet() && !pkChr->IsMount() && pkChr->GetRider() == NULL)
#else
		if (pkChr->GetRaceNum() != 30000 && pkChr->IsNPC() && !pkChr->IsPet() && !pkChr->IsNewPet() && !pkChr->IsMount() && pkChr->GetRider() == NULL)
#endif
#else
...
ACMD(do_state)
{
	...
	for (int i = 0; i < MAX_PRIV_NUM; ++i)
		if (CPrivManager::instance().GetPriv(tch, i))
		{
			...
			if (iByEmpire)
				ch->ChatPacket(CHAT_TYPE_INFO, "%s for empire : %d", LC_TEXT(DEFAULT_LANGUAGE, c_apszPrivNames[i]), iByEmpire);

			if (iByGuild)
				ch->ChatPacket(CHAT_TYPE_INFO, "%s for guild : %d", LC_TEXT(DEFAULT_LANGUAGE, c_apszPrivNames[i]), iByGuild);

			if (iByPlayer)
				ch->ChatPacket(CHAT_TYPE_INFO, "%s for player : %d", LC_TEXT(DEFAULT_LANGUAGE, c_apszPrivNames[i]), iByPlayer);
		}
}
...
struct notice_map_packet_func
{
	...
	int m_mapIndex;
	bool m_bBigFont;
#ifdef ENABLE_MULTILANGUAGE // For OX Event
	bool m_bTranslate; 
	notice_map_packet_func(const char* str, int idx, bool bBigFont, bool bTranslate) : m_str(str), m_mapIndex(idx), m_bBigFont(bBigFont), m_bTranslate(bTranslate)
#else
...
#ifdef ENABLE_MULTILANGUAGE // For OX Event
		if (m_bTranslate)
			d->GetCharacter()->ChatPacket(m_bBigFont == true ? CHAT_TYPE_BIG_NOTICE : CHAT_TYPE_NOTICE, "%s",  LC_TEXT(d->GetCharacter()->GetLang(), m_str));
		else
...
void SendNoticeMap(const char* c_pszBuf, int nMapIndex, bool bBigFont, bool bTranslate)
{
	const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
#ifdef ENABLE_MULTILANGUAGE // For OX Event
	std::for_each(c_ref_set.begin(), c_ref_set.end(), notice_map_packet_func(c_pszBuf, nMapIndex, bBigFont, bTranslate));
#else
...
ACMD(do_monarch_notice)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "군주만이 사용 가능한 기능입니다"));
	}
}
...
typedef enum do_set_types_s {GOLD, RACE, SEX, JOB, EXP, MAX_HP, MAX_SP, SKILL, ALIGNMENT, ALIGN, WON, GAYA} do_set_types_t;
}
#else
namespace DoSetTypes{
typedef enum do_set_types_s {GOLD, RACE, SEX, JOB, EXP, MAX_HP, MAX_SP, SKILL, ALIGNMENT, ALIGN, WON} do_set_types_t;
}
#endif
...
ACMD(do_makeguild)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "적합하지 않은 길드 이름 입니다."));
		return;
	}

	gm.CreateGuild(cp);
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "(%s) 길드가 생성되었습니다. [임시]"), cp.name);
}
...
ACMD(do_event_flag)
{
	...
	if (!strcmp(arg1, "group_match_giris"))
	{
		if (value != 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Entradas de grupo abierta");
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Entradas de grupo cerrada");
		}
	}
	...
	else if (!strcmp(arg1, "group_notice"))
	{
		if (value != 0)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "<Busqueda Grupal1");
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "<Busqueda Grupal2");		
		}
	}
	#endif
	#endif
...
ACMD(do_gwlist)
{
	ch->ChatPacket(CHAT_TYPE_NOTICE, LC_TEXT(TRANSLATE_LANGUAGE, "현재 전쟁중인 길드 입니다"));
...
ACMD(do_guild_state)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "%s: 존재하지 않는 길드 입니다."), arg1);
	}
}
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "그런 이름 또는 번호의 길드가 없습니다."));
		else
		{
			char buf[1024+1];
...
ACMD(do_block_chat_list)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "그런 명령어는 없습니다"));
		return;
	}
...
ACMD(do_ban)
{
	...
	if (iDuration <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Duration can't be 0 or minuts.");
		return;
	}
	...
	std::auto_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account.account SET availDt = FROM_UNIXTIME(UNIX_TIMESTAMP(CURRENT_TIMESTAMP()) + %i), address = '%s' WHERE id = %d", iDuration * 3600, szReason, tch->GetDesc()->GetAccountTable().id));
		...
		case 'c':
			{
				...
				if (!t)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "존재하지 않는 건물입니다."));
					return;
				}
				...
				if (t->dwGroupVnum)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "같이 지을 수 없는 종류의 건물이 지어져 있습니다."));
						return;
					}
				}
				...
				if (t->dwDependOnGroupVnum)
				{
					...
					//		if (dependent)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "건설에 필요한 건물이 지어져 있지 않습니다."));
							return;
						}
					}
				}
				...
				if (test_server || GMLevel == GM_PLAYER)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "건물 비용 정보 이상으로 건설 작업에 실패했습니다."));
						return;
					}

					if (ch->GetGold() < (int)t->dwPrice)
					{
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "건설 비용이 부족합니다."));
						return;
					}
					...
					for (i = 0; i < OBJECT_MATERIAL_MAX_NUM; ++i)
					{
						...
						{
							ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "자재가 부족하여 건설할 수 없습니다."));
							return;
						}
					}
				}
				...
				{
					if (test_server)
						ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "건물을 지을 수 없는 위치입니다."));
					return;
				}
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "존재하지 않는 캐릭터 입니다."));
		return;
	}
...
ACMD(do_end_duel)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "대련 강제 종료 실패"));
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "대련 강제 종료 성공"));
	}
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "<파티> 파티에서 나가셨습니다."));
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "<파티> 파티에서 나가셨습니다."));
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "대련이 성공적으로 시작 되었습니다."));
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "대련 시작에 문제가 있습니다."));
		}
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "대련자가 없습니다."));
	}
}
...
ACMD(do_stat_plus_amount)
{
	...
	if (ch->IsPolymorphed())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "둔갑 중에는 능력을 올릴 수 없습니다."));
		return;
	}
	...
	if (nRemainPoint <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "남은 스탯 포인트가 없습니다."));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "남은 스탯 포인트가 적습니다."));
		return;
	}

	if (nPoint < 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "값을 잘못 입력하였습니다."));
		return;
	}
	...
	switch (subcmd)
	{
		...
		default :
			ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "명령어의 서브 커맨드가 잘못 되었습니다."));
			return;
			break;
	}
#endif
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "플레이어 %d 와 플레이어  %d를 파혼시킵니다.."), pids.pid1, pids.pid2);
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "각제국 진행 정보"));
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "선택 맵 정보 성지 %d 통로 %d %d %d"), GetSungziMapIndex(), GetPassMapIndex(1), GetPassMapIndex(2), GetPassMapIndex(3));
...
{
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "나의 삼거리 진행정보"));
...
ACMD(do_set_stat)
{
	...
	else
	{
		...
		case POINT_HT:
			...
			{
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "Cannot set stat under initial stat."));
				return;
			}
			n = 0;
			break;
...
#ifdef KYGN_AURA
ACMD(do_aura_paneli_kapat)
{
	ch->PaneliKapat();
}

ACMD(do_aura_gelistir_button)
{
	ch->AuraGelistir();
}

ACMD(do_aura_yukselt_button)
{
	ch->AuraYukselt();
}
ACMD(do_aura_emis_button)
{
	ch->AuraEmis();
}
ACMD(do_aura_clear)
{
	...
	if (slot == 0)
		ch->AurayiSlotaEkleme(0);
	else if (slot == 1)
		ch->MaterialiSlotaEkleme(0);
...
ACMD(do_aura_add)
{
	...
	str_to_number(position, arg1);

	if (position > 1000)// envanter max slot sayi
		return;

	if (ch->GetInventoryItem(position) != NULL)
	{
		if (ch->GetWindow() == 3)
		{
			if (ch->GetInventoryItem(position)->GetVnum() >= 49001 && 49006 >= ch->GetInventoryItem(position)->GetVnum())
				ch->AurayiSlotaEkleme(ch->GetInventoryItem(position));
			else if (ch->GetInventoryItem(position)->GetType() == ITEM_ARMOR && ch->GetInventoryItem(position)->GetSubType() != ARMOR_BODY)
				ch->MaterialiSlotaEkleme(ch->GetInventoryItem(position));
		}
		else
		{
			if (ch->GetInventoryItem(position)->GetVnum() >= 49001 && 49006 >= ch->GetInventoryItem(position)->GetVnum())
				ch->AurayiSlotaEkleme(ch->GetInventoryItem(position));
			else if (ch->GetInventoryItem(position)->GetVnum() == 49990)
				ch->MaterialiSlotaEkleme(ch->GetInventoryItem(position));
		}
	}
