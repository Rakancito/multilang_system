...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		...
		{
			pkChar->ChatPacket(CHAT_TYPE_INFO, "%d %s %s", m_vec_quiz[i][j].level, m_vec_quiz[i][j].Quiz, m_vec_quiz[i][j].answer ? LC_TEXT(DEFAULT_LANGUAGE, "참") : LC_TEXT(DEFAULT_LANGUAGE, "거짓"));
		}
	}

	pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "총 퀴즈 수: %d"), c);
	return true;
}
...
EVENTFUNC(oxevent_timer)
{
	...
	switch (flag)
	{
		case 0:
			...
			SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "10초뒤 판정하겠습니다."), OXEVENT_MAP_INDEX, true, true);
#else
		...
		case 1:
#ifdef ENABLE_MULTILANGUAGE
			SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "정답은"), OXEVENT_MAP_INDEX, true, true);
#else
...
#ifdef ENABLE_MULTILANGUAGE
				SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "O 입니다"), OXEVENT_MAP_INDEX, true, true);
#else
...
#ifdef ENABLE_MULTILANGUAGE
				SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "X 입니다"), OXEVENT_MAP_INDEX, true, true);
#else
				SendNoticeMap(LC_TEXT("X 입니다"), OXEVENT_MAP_INDEX, true);
#endif
			}
#ifdef ENABLE_MULTILANGUAGE
			SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "5초 뒤 틀리신 분들을 바깥으로 이동 시키겠습니다."), OXEVENT_MAP_INDEX, true, true);
#else
...
#ifdef ENABLE_MULTILANGUAGE
			SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "다음 문제 준비해주세요."), OXEVENT_MAP_INDEX, true, true);
#else
			SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "다음 문제 준비해주세요."), OXEVENT_MAP_INDEX, true);
#endif
			flag = 0;
			break;
	}
...
#ifdef ENABLE_MULTILANGUAGE
	SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "문제 입니다."), OXEVENT_MAP_INDEX, true, true);
	SendNoticeMap(m_vec_quiz[level][idx].Quiz, OXEVENT_MAP_INDEX, true, true);
	SendNoticeMap(LC_TEXT(TRANSLATE_LANGUAGE, "맞으면 O, 틀리면 X로 이동해주세요"), OXEVENT_MAP_INDEX, true, true);
#else
...
bool COXEventManager::CheckAnswer(bool answer)
{
	...
	for (; iter != m_map_attender.end();)
	{
		...
		if (pkChar != NULL)
		{
			...
			{
				pkChar->ChatPacket(CHAT_TYPE_INFO, LC_TEXT(TRANSLATE_LANGUAGE, "정답입니다!"));
