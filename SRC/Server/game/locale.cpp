...
LocaleStringMapType localeString[254];
std::string arstLocaleStringNames[254];
...
const char * locale_find(BYTE blang, const char *string)
{
	...
	if (blang >= MAX_LANGUAGES)
	{
		...
		if( iter == localeString[DEFAULT_LANGUAGE].end() )
		{
			...
			sys_err("LOCALE_ERROR 1: \"%s\" lang_type: %d;", string, blang);
			return s_line;
		}
	...
	else
	{
		...
		if( iter == localeString[blang].end() )
		{
			...
			sys_err("LOCALE_ERROR 2: \"%s\" lang_type: %d;", string, blang);
			return s_line;
		}
...
void locale_init(BYTE blang, const char *filename)
{
	...
	do
	{
		...
		if (*tmp == '"')
		{
			...
			locale_add(blang, (const char**)strings);
