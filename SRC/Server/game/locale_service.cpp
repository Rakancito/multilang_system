...
std::string g_stLocaleFinename_path = "locale/mexico/"; // Change "mexico" for your country
std::string g_stLocaleFinename_lang = "";
...
void LocaleService_LoadLocaleStringFile()
{
	...
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT lang FROM common.lang"));
	...
	{
		g_stLocaleFinename_lang = g_stLocaleFinename_path + "locale_string_" + row[0] + ".txt"; 
		fprintf(stdout, "System Multi Load:%s -> %d \n", g_stLocaleFinename_lang.c_str(), iPos);
		locale_init(iPos, g_stLocaleFinename_lang.c_str());
		iPos++;
	}		
}
#else
