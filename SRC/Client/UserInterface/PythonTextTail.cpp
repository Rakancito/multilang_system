...
void CPythonTextTail::ArrangeTextTail()
{
	...
	for (itor = m_CharacterTextTailList.begin(); itor != m_CharacterTextTailList.end(); ++itor)
	{
		...
		if (pCombatZoneRank)
		{
		#if defined(__NEW_TITLE__) || defined(ENABLE_TITLE_SYSTEM)
			if (pGuildNameInstance && pMrVegaS_Titan2)
			{
				pCombatZoneRank->SetPosition (pTextTail->x - pCombatZoneRank->GetWidth()/2, pTextTail->y - (c_fyMarkPosition * 4) + 3.0f);
			}
			else if (pGuildNameInstance && !pMrVegaS_Titan2)
			{
				pCombatZoneRank->SetPosition (pTextTail->x - pCombatZoneRank->GetWidth()/2, pTextTail->y - (c_fyMarkPosition * 4) + 15.0f);
			}
			else if (pMrVegaS_Titan2 && !pGuildNameInstance)
			{
				pCombatZoneRank->SetPosition (pTextTail->x - pCombatZoneRank->GetWidth()/2, pTextTail->y - (c_fyMarkPosition * 4) + 15.0f);
			}
			else
...
#ifdef ENABLE_SHOW_LEADER_GUILD
std::string CPythonTextTail::IsLeaderOnGuild(std::string * strConvertGuildName)
{
/*************************************
* Date            : 08.01.2017
* FileName        : PythonTextTail.cpp
* TitleName       : IsLeaderOnGuild
* Author          : VegaS
* Description     : 
	When you are leader on your guild will appear in front [leader].
	You can and enable color on textLine when you are leader with ENABLE_SHOW_LEADER_GUILD_COLOR.
*/
	#define LEADER_GUILD_COLOR  "|cFFd0ffcc"
	CPythonGuild::TGuildInfo & rGuildData = CPythonGuild::Instance().GetGuildInfoRef();
	CPythonGuild::TGuildMemberData * pInfo;
...
void CPythonTextTail::RegisterCharacterTextTail(DWORD dwGuildID, DWORD dwVirtualID, const D3DXCOLOR & c_rColor, float fAddHeight)
{
	...
	if (0 != dwGuildID)
	{
		...
		if (!CPythonGuild::Instance().GetGuildName(dwGuildID, &strGuildName))
			strGuildName = "Noname";
#ifdef ENABLE_SHOW_LEADER_GUILD
		std::string strConvertGuildName = CPythonTextTail::Instance().IsLeaderOnGuild(&strGuildName);
#endif
...
#ifdef ENABLE_SHOW_LEADER_GUILD
		prGuildNameInstance->SetValue(strConvertGuildName.c_str());
#else
		prGuildNameInstance->SetValue(strGuildName.c_str());
#endif
