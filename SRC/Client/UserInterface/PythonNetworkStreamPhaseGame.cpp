...
void CPythonNetworkStream::__RefreshPrestigeWindow()
{
	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "RefreshPrestige", Py_BuildValue("()"));
}
#endif
...
#ifdef KYGN_AURA
			case HEADER_GC_KYGN_AURA:
				ret = RecvKygnAuraPacket();
				break;
#endif
...
bool CPythonNetworkStream::SendCharacterStatePacket(const TPixelPosition& c_rkPPosDst, float fDstRot, UINT eFunc, UINT uArg)
{
	...
	CInstanceBase* pkInstMain = rkChrMgr.GetMainInstancePtr();
	if (pkInstMain){
		if (!pkInstMain->IsGoing() && pkInstMain->__CanCollision()){
			__SendHack("Wall hack Detect");
		}
	}
...
bool CPythonNetworkStream::SendExchangeItemAddPacket(TItemPos ItemPos, WORD byDisplayPos)
{
	if (!__CanActMainInstance())
		return true;
...
bool CPythonNetworkStream::SendExchangeItemDelPacket(WORD pos)
{
...
{
	TPacketGCDamage DamagePacket;

	if (!Recv(sizeof(TPacketGCDamage), &DamagePacket))
	{
		Tracen("Recv Target Packet Error");
		return false;
	}

	CInstanceBase * pInstTarget = CPythonCharacterManager::Instance().GetInstancePtr(DamagePacket.dwVID);
	bool bSelf = (pInstTarget == CPythonCharacterManager::Instance().GetMainInstancePtr());
	bool bTarget = (pInstTarget==m_pInstTarget);
	if (pInstTarget)
	{
		if(DamagePacket.damageinfo >= 0)
			pInstTarget->AddDamageEffect(DamagePacket.damageinfo,DamagePacket.flag,bSelf,bTarget);
		else
...
bool CPythonNetworkStream::RecvTargetPacket()
{
	...
	if (pInstPlayer && pInstTarget)
	{
		if (!pInstTarget->IsDead())
		{
			...
			#endif
			if(TargetPacket.icurHP > 0)
				bHPPercent = MINMAX(0, (TargetPacket.icurHP * 100) / TargetPacket.iMaxHP, 100);
			
			if (pInstTarget->IsBuilding() || pInstTarget->IsPC())
			{
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "CloseTargetBoardIfDifferent", Py_BuildValue("(i)", TargetPacket.dwVID));
				
				if(pInstTarget->IsPC())
#ifdef ELEMENT_TARGET
					PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard_PC", Py_BuildValue("(iiiii)", TargetPacket.dwVID, bHPPercent, TargetPacket.icurHP, TargetPacket.iMaxHP, TargetPacket.bElement));
#else
					PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard_PC", Py_BuildValue("(iiii)", TargetPacket.dwVID, bHPPercent, TargetPacket.icurHP, TargetPacket.iMaxHP));
#endif
			}
			else if (pInstPlayer->CanViewTargetHP(*pInstTarget))
			{
#ifdef ELEMENT_TARGET
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard", Py_BuildValue("(iiiii)", TargetPacket.dwVID, bHPPercent, TargetPacket.icurHP, TargetPacket.iMaxHP, TargetPacket.bElement));
#else
				PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "SetHPTargetBoard", Py_BuildValue("(iiii)", TargetPacket.dwVID, bHPPercent, TargetPacket.icurHP, TargetPacket.iMaxHP));
#endif
			}
			else
			{
...
#ifdef KYGN_AURA
bool CPythonNetworkStream::RecvKygnAuraPacket()
{
	TPacketGCKygnAura KygnAuraGC;

	if (!Recv(sizeof(TPacketGCKygnAura), &KygnAuraGC))
	{
		Tracen("Recv KygnAuraGC Packet Error");
		return false;
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "AuraPanel",
		Py_BuildValue("(iiii)", KygnAuraGC.Pencere, KygnAuraGC.AuraLevel, KygnAuraGC.AuraPos, KygnAuraGC.MaterialPos));
	return true;
}
#endif
...
bool CPythonNetworkStream::AttackPSendNew(UINT uMotAttack, DWORD dwVIDVictim)
{
	...
	if (!SendSpecialNew(sizeof(kPacketAtk), &kPacketAtk))
	{
...
bool CPythonNetworkStream::SendSpecialNew(int nLen, void * pvBuf)
{
...
#else
#ifdef KASMIR_PAKET_SYSTEM
bool CPythonNetworkStream::SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock, DWORD dwKasmirNpc, BYTE bKasmirBaslik)
#else
...
#ifdef KASMIR_PAKET_SYSTEM
		if (pInstance && (pInstance->IsPC() || (pInstance->GetRace() >= 30000 && pInstance->GetRace() <= 30007)))
#else		
...
#ifdef KASMIR_PAKET_SYSTEM
			Py_BuildValue("(ibs)", p.dwVID, p.bShopKasmirTitle, p.szSign)
#else
...
bool CPythonNetworkStream::SendClientVersionPacket()
{
	...
	if (LocaleService_IsEUROPE() && false == LocaleService_IsYMIR())
	{
		...
		strncpy(kVersionPacket.timestamp, "1215955207", sizeof(kVersionPacket.timestamp)-1); // # python time.time 앞자리
...
bool CPythonNetworkStream::SendChangeLanguage(BYTE blang)
{

	TPacketCGChangeLang pack;

	pack.header = HEADER_CG_CHANGE_LANGUAGE;
	pack.blang = blang;
	if (!Send(sizeof(pack), &pack))
	{
		return false;
