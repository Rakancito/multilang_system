...
class CPythonNetworkStream : public CNetworkStream, public CSingleton<CPythonNetworkStream>
{
	...
	public:
		...
		bool SendSpecialNew(int nLen, void * pvBuf);
		...
		bool AttackPSendNew(UINT uMotAttack, DWORD dwVIDVictim);
		...
		bool RecvTransmutation();
		bool SendTransmutationAddSlot(int selectedSlot, int attachedSlot);
		bool SendTransmutationDeleteSlot(int selectedSlot);
		bool SendTransmutationMake();
		bool SendTransmutationCancel();
#endif
		...
		bool SendExchangeItemAddPacket(TItemPos ItemPos, WORD byDisplayPos);
		...
		#else
#ifdef KASMIR_PAKET_SYSTEM
		bool SendBuildPrivateShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock, DWORD dwKasmirNpc, BYTE bKasmirBaslik);
#else
		...
		bool SendChangeLanguage(BYTE blang);
#endif
		// Client Version
