...
void CInstanceBase::AddDamageEffect(DWORD damageinfo,BYTE flag,BOOL bSelf,BOOL bTarget)
{
	if(CPythonSystem::Instance().IsShowDamage())
	{
		...
		sDamage.damageinfo = damageinfo;
		sDamage.flag = flag;
		m_DamageQueue_.push_back(sDamage);
	}
}

void CInstanceBase::ProcessDamage_()
{
	if(m_DamageQueue_.empty())
		return;

	SEffectDamage sDamage = m_DamageQueue_.front();

	m_DamageQueue_.pop_front();

	DWORD damageinfo = sDamage.damageinfo;
...
UINT CInstanceBase::GetNameColorIndex()
{
		...
		if (GetRace() == 30000 || GetRace() == 30001 || GetRace() == 30002 || GetRace() == 30003 || GetRace() == 30004 || GetRace() == 30005 ||
		GetRace() == 30006 || GetRace() == 30007)
...
void CInstanceBase::AttachTextTail()
{
	...
	if (IsPC())
		UpdateTextTailCountryFlag(m_bCountry);
#endif
}

#ifdef ENABLE_MULTILANGUAGE
void CInstanceBase::UpdateTextTailCountryFlag(BYTE c_bCountry)
{
	const char* c_sCountry = ReplaceSwitchCountry(c_bCountry);
	CPythonTextTail::Instance().AttachCountryFlag(GetVirtualID(), c_sCountry);
}
const char* CInstanceBase::ReplaceSwitchCountry(BYTE c_bCountry)
{
	// FOR CHANGE COUNTRY
	const char * cCountry = DEFAULT_LANGUAGE;
	switch (c_bCountry)
	{
	case 0:
		cCountry = "en";
...
DWORD CInstanceBase::__AttachEffect(UINT eEftType)
{
	...
	else
	{
		...
		else if (0 == rstrBoneName.compare("PART_AURA"))
		{
			if (m_GraphicThingInstance.GetAttachingBoneName(CRaceData::PART_AURA, &c_szBoneName))
			{
				return m_GraphicThingInstance.AttachEffectByID(0, c_szBoneName, ms_adwCRCAffectEffect[eEftType]);
			}
		}
